...
public class InboundEventHandler {
    ...
    public void getMessage(Message<?> message) {
        ...
        var eventType = (String) headers.get(EVENT_TYPE);
        if (eventType == null || eventType.isEmpty()) {
            log.error("Invalid event, " + EVENT_TYPE + " is null");
            throw new RuntimeException("Invalid event, " + EVENT_TYPE + " is null");
        }
    ...
    private void handleEvent(String payload,String eventType){
        EventHandlerWrapper eventHandlerWrapper = handlerMap.get(eventType);
        try {
            Object event = objectMapper.readValue(payload, eventHandlerWrapper.eventClass);
